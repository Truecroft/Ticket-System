...
def view_tickets(status):
    if status == 'all':
        ...
        if len(tickets) < 1:
            flash("You have no tickets which you have created. Returning to the Home page.", category='error')
            return redirect(url_for('routes.home'))
    else:
...
def view_open_tickets():
    ...
    if len(tickets) < 1:
        flash("You currently have no open tickets. Returning to Home Page", category='error')
        return redirect(url_for('routes.home'))
